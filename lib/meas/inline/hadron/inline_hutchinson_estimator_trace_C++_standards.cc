/*! \file
 * \brief Inline Hutchinson Trace Estimator
 */
#include <cmath>
...
void z2_src_t(QDP::LatticeFermion & a)
{
    a = zero;
    QDP::LatticeReal rnd;
    QDP::LatticeReal ar, ai;
    QDP::LatticeColorVector colorvec = zero;
    ...
    for (int spin_index = 0; spin_index < Ns; ++spin_index)
    {
        ...
        {
            random(rnd);
            ar = where(rnd > 0.5, QDP::LatticeReal(1), QDP::LatticeReal(-1));
            random(rnd);
            ai = where(rnd > 0.5, QDP::LatticeReal(1), QDP::LatticeReal(-1));
            QDP::LatticeComplex c = cmplx(ar, 0);
...
namespace Chroma
{
    ...
    typedef multi1d<QDP::LatticeColorMatrix> P;
    typedef multi1d<QDP::LatticeColorMatrix> Q;
    ...
    void read(XMLReader &xml, const std::string &path, InlineHutchinsonTraceEstimatorParams::NamedObject_t &input)
    {
        ...
        read(inputtop, "prop_1", input.prop_1); //Add propagator ID 1
        read(inputtop, "prop_2", input.prop_2); //Add propagator ID 2
        read(inputtop, "prop_r", input.prop_r); //Add propagator ID result
    }
                                            ...
                                            XMLWriter &xml_out)
    {
        ...
        LatticeReal rnd1;
        random(rnd1); //Save a random number in each lattice space
        ...
        { // i in range(0,100)
            z2_src_t(entrada); //Build up z2 noise vector
            salida = entrada;  //Copy
            ...
            temp[i] = innerProduct(entrada, salida); //Dot product
